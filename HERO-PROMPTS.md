...
the MP4 — browsers can only seek MP4 keyframes, which stutters):

```bash
cd ~/liquid-web/sites/tam-pharmacy
ffmpeg -i assets/hero-loop.mp4 -vf "fps=24,scale=1280:720" -q:v 4 \
  assets/frames/frame-%04d.jpg
ls assets/frames | wc -l    # set TOTAL_FRAMES in index.html to this number
```
